...
def num_sol( at, y0, par):
    ...
    for i in range( nSteps-1):
        ...
        fn1 = av_hat[i]    #dPar['F']*np.cos(dPar['w']*at[i])
...
#ay_ana1   = preFacAna * ( dPar['F']/((dPar['w0']**2 - dPar['w']**2)*( np.cos(dPar['w']*a_t) - np.cos(dPar['w0']*a_t) )))
# ay_ana1   = preFacAna * (np.cos(dPar['w0']*a_t) )

#:TODO rewrite the analytical solution as product of sines
#ay_ana1 = -???? * ( np.sin( ????)  * np.sin( ??????))
# envelope - slow frequency
ay_ana2 =  -2*preFacAna*(a_t)/10.5 
#--------------------------------3---------------------------------------
#                      numerical solutions
#------------------------------------------------------------------------
...
ax.plot(a_t,   aU_num, 'r--', lw = 1, label = 'ana - full')
...
#plt.figure(2)
q = plt.subplot(111)
q.plot(a_t, aU_num, 'g-', lw = 3, label = 'diff between ana and num')

q.legend( loc = 'upper left')
plt.show()