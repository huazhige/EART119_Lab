...
x_root =  my_Secant( f1, x0, x0+10) #root for f1
x2_root = my_Secant( f2, x0, x0+10) #root for f2
x3_root = my_Secant( f3, x0, x0+10) #root for f3
...
plt.grid(True)
plt.xlabel( 'x')
plt.ylabel( 'Fct Values f2(x)')
plt.show()

a_x = np.linspace( xmin, xmax, 1000)
plt.figure(3)
plt.plot( a_x, f3(a_x), 'k-')
plt.plot( [x3_root], [f3(x3_root)], 'r*', ms = 14)
plt.plot( [xmin,xmax], [0,0], 'r--')
plt.grid(True)
plt.xlabel( 'x')
plt.ylabel( 'Fct Values f3(x)')
plt.show()
