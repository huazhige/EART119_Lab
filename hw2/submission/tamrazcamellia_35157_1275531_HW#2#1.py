...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 23 22:05:00 2019
...
#------------------------------------------------------------------------
data_dir   = ???
file_eq    = 'Data/seism_OK.txt'
file_well  = 'Data/injWell_OK.txt' #intervals for plotiing the eq and wells 
...
DecYear= YR +(MO-2)/12 + (DY-1)/365.25+HR/(365.25*24)+MN/(365.24)+SC/(365.25*24*3600)
...
if dPar['showRate'] == True:
    ...
    aBin, aRate = seid_utils.eqRate(at, k_win)
    print(aBin, aRate)
    ...
    fig_2= plt.figure(2)
    ax2= plot.subpolot( at_eq, aV, 'bx' , ms = 2)
...
for i in range( at_bin.shape[0]-1):
    ...
    plt.plot( aX, aY, 'ro' , ms = %, new = 1.5, mfc = 'none')
    #pltplot( aX, aY, file_well, 'bo', ms= 2, new = 1.5, mfc = 'none')
    plt.show()
