...
# U_top = Ubar + delta(Ubar) * np.cos(t * w)
# If we assume Ubar = 0, then U_top = delta(Ubar) * np.cos(t * w)
# delta(Ubar) = np.linspace(-50, 50, 100)
# For the formula below, the amplitude is the value being multiplied to cosine
# As such, we call Ubar = fAmp
...
def fct_BC_L( t, fAmp, w):
    ...
    fAmp = 50. # The amplitude at -50 will be when cos(t * w) = -1
...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    #return fAmp*2 * np.e**(-x * np.sqrt(w / (2*fAlpha))) * 
    #np.cos( -x * np.sqrt(w / (2*fAlpha)) + w*t)
...
for n in range( len(a_t)):# loop over time increments
    ...
    for i in range( 1, i_Nx-1): #spatial loop without boundary nodes
        ...
        a_dUdt[i] = f_Alpha*( (a_U[i+1] - a_U[i-1]) / (2 * f_dx))
    ...
    a_Unew = a_U + f_dt * f_Alpha * ((a_U[i+1] - (2 * a_U[i]) + a_U[i-1]) / f_dx**2)
    # update the temperature profile for the next time step, otherwise only BC and IC are shown
    a_U    = a_Unew

    ##D##---TODO: compute exact solution
    a_Uana = 0. + f_Uin*2 * np.e**(-a_x * np.sqrt(f_W / (2*f_Alpha))) * \
    np.cos( -a_x * np.sqrt(f_W / (2*f_Alpha)) + f_W*a_t)
    #=========================3===========================================
