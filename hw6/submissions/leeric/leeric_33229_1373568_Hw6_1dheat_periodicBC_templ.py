...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    return delta_u + fAmp*np.exp(-x*np.sqrt(w/(2*fAlpha)))* \
...
for n in range( len(a_t)):# loop over time increments
    ...
    if (n+1)%plot_step == 0:
        ...
        ax1.plot( a_U, -a_x, 'b-', ms = 3, mew =1.5, mfc = 'w', label = 'u(x,t)-num')
...
depth = 2.440677966101695
print("depth where temperature varies by 1 degree", a_x[n_iter])
print("when surface temperature fluctuates by 80 degrees, I must dig: ", depth, "meters")
