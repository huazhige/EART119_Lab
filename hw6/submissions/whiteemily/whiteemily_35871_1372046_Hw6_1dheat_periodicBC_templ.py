...
1. The depth for less than 1 degree is 3-4m and 0-1m
3. You would have to dig until past 2m
"""
...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    prefac = fAmp/(np.sqrt(1+4*t*fAlpha/w**2))
    return prefac*np.exp(-x**2/(w**2 + 4*t*fAlpha))
...
# boundary condition
f_Uin   = 1 # amplitude of temperature oscillation
f_Uout  = 80.   # in degree C
