...
def fct_BC_L( a_t, f_Uin, f_W):
    ...
    return  f_Uin * np.cos(a_t * f_W)

def fct_u_exact( a_dUdt, a_x, f_Alpha, f_Uin, f_W):
    """
    ...
    :return:
    """
    return f_U0 + f_Uin * np.exp(-a_x*np.sqrt(f_W/(2*f_Alpha)))*np.cos(-a_x*np.sqrt(f_W/(2*f_Alpha))+f_W * a_dUdt)
...
# boundary condition
f_Uin   =   50 # C, amplitude of temperature oscillation
f_Uout  =   1   # in degree C
...
for n in range( len(a_t)):# loop over time increments
    ...
    a_Uana = fct_u_exact( a_dUdt, a_x, f_Alpha, f_Uin, f_W)
    #=========================3===========================================
    ...
    # A: After roughly ~ 2.5m you would be safe from the temperature fluctuations
    
    # C: After roughly ~ 2.5m you would be safe.  
    #    The depth does not depend on the magnitude of the changes.
