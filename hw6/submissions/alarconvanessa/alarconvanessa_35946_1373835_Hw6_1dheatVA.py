...
Solve 1D-Heat equation for change in planetary temperature with depth from surface
    ...
    1) Based on the animation, at about 2.5m below the surface the temperature
    does not change much (possibly between -1 and 1 deg C)
    
    2) plot
    
    3) Change line 56, fUin to 80 instead of 50. You'd have to dig a little 
    further than 2.5m. 
...
#============ boundary condition ===================
f_Uin   = 50     # 80 for 3  # amplitude of temperature oscillation
...
for n in range( len(a_t)):# loop over time increments
    ...
    # integration step: forward difference Formula to get u(x,t)
