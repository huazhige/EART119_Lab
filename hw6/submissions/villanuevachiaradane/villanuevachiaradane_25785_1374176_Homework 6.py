...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    return fAmp + fAmp*np.exp(-x*np.sqrt(w/2*fAlpha))*np.cos(-x*np.sqrt(w/2*fAlpha) + w*t)
...
#=====================================================================
"""
    3. If the temperatures fluctuate by +/- 80 degrees Celsius,
        the level of suggested depth to dig should be the same
        as it were if it fluctuated by +/- 50 degrees Celsius,
        which is around 2 - 2.5 meters.
"""
