...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    :return:
    """
    return fAmp*np.e**(-x*np.sqrt( w/( 2*fAlpha)))*np.cos( -x*np.sqrt( w/( 2*fAlpha)) + w*t)
...
# boundary condition
f_Uin   = 50 # amplitude of temperature oscillation [80 for Problem 3]
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if (n+1)%plot_step == 0:
        ax1.cla()
        ax1.set_title( 'Time Step: %.2f [day]'%( a_t[n]/(24*3600)))
...
"""
The numerical solution and the analytical solution are slightly off in the
beginning, but get closer over time. At first, the exact solution lags 
behind the numerical solution, but they sync up after about 0.75 of a day.
"""
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
Replacing the +/- 50 C in the code with +/- 80 C doesn't yield any significant
difference in the graph's return to the y-axis. In both cases, there will be 
no temperature fluctations at between 2 and 2.5 meters underground.

The difference that replacing the 50 with 80 makes is a greater lag between
the numerical and analytical solutions, and slower transitions between the 
temperature fluctuations upon the surface.
"""
