...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    return fAmp*np.exp(-x * np.sqrt(w/2*f_Alpha))*np.cos(-x* np.sqrt(w/2*f_Alpha) + w* t)
...
##The expected temperature variation is less than or about 1 at a depth of about 2-2.5 m
