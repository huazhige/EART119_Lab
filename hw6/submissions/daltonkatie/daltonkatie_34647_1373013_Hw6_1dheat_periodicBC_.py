...
#======================================================================
"""
1. At a depth of about 2 meters, there is extected to be less 
    than 1 degree variation in temperature.
3. For fluctuations of 80 degrees, a depth of about 2 meters will also
    have less than 1 degree of temperature variation.
"""
...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    sqrt = np.sqrt(w/(2*fAlpha))
    return fAmp*np.exp(-x*(sqrt))*np.cos(-x*(sqrt)+w*t)
...
for n in range( len(a_t)):# loop over time increments
   ...
   # print(a_Uana)
    ...
    if (n+1)%plot_step == 0:
        ...
        ax1.plot( a_Uana, -a_x, 'r--', lw = 1.5, ms = 3, mew =1.5, mfc = 'w', label = 'u(x,t)-exact')
