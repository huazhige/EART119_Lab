...
for n in range( len(a_t)):# loop over time increments
    ...
    while (np.abs(a_Unew[depthIdx]-f_U0)>1):
        depthIdx += 1
    depth = depthIdx*f_dx
 ...
 #       ax1.plot(  [-f_Uin, f_Uin], [-2*r_diff, -2*r_diff], 'r--', label = '2 x l_diff')
        ...
        ax1.plot([-50,50], [-depth,-depth], 'r-')
        depthName ='safe depth: ' + str(round(depth,2)) + 'm'
        ax1.text(-40,-depth+0.1,depthName)
...
print('depth with 1 degree temperature variation ', str(round(depth,2)))
fileName = 'AsafeDepth_dt'+str(int(f_dt))+'.png'
