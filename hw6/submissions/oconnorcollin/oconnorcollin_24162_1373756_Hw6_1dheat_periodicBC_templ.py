...
for n in range( len(a_t)):# loop over time increments
    ...
    #=====================================================================
    print 'time step: ', a_t[n]/(24*3600), 'days'#,(n+1)/plot_step,(n+1)%plot_step
    if (n+1)%plot_step == 0:
        ax1.cla()
        ax1.set_title( 'Time Step: %.2f [days]'%( a_t[n]/(24*3600)))
        ...
        ax1.plot( a_Uana, -a_x, 'b--', lw = 1.5, label = 'u(x,t)-exact')
...
print """Question 1: For depths equal to or greater than 2.5 meters, the 
temperature variation in the soil is less than one degree. """
print
print """Question 2: As you can see from the graph, the analytical 
solution almost lines up perfectly with the numerical solution"""
print
print """Question 3: If we change variable f_Uin (the amplitude of the
temperature oscillation) to equal 80 (degrees Celcius), the max 
depth that we would have to dig so that temperature variations 
beneath the surface don't oscillate beyond 1 degree would still 
be 2.5 meters. This means that regardless of temperature 
variation for temperatures higher than 80 degrees Celcuis, the 
minimum max depth we'd need to dig will always be 2.5 meters."""
