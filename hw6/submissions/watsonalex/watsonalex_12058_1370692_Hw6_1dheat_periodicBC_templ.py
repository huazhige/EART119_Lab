...
def fct_u_exact( t, x, fAlpha, fAmp, w):
    ...
    return f_U0 + fAmp*np.exp( -x*np.sqrt(w/(2*fAlpha)))*np.cos(-x*np.sqrt( w/(2*fAlpha)) + w*t)
...
for n in range( len(a_t)):# loop over time increments
    ...
    if (n+1)%plot_step == 0:
        ...
        ax1.plot(  [1,1], [-a_x.max(), 0], '--', color = ' .5')
        ax1.plot(  [-1,-1], [-a_x.max(), 0], '--', color = ' .5')
...
"""
 part 3: when the temp fluctuation is +-50 degrees C, there is a temp variation of <1 degree C
 at depths of around 2 meters and deeper. When the temp fluctuation is increased to +-80 degrees C,
 this depth is lowered to around 2.3 meters or so.
"""
