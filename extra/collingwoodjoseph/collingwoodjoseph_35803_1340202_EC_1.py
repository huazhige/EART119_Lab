# -*- coding: utf-8 -*-
"""
Extra credit problem 1
"""

import numpy as np

#FUNCTION DEFINITIONS
def f_t(t):
    return 3*t**2*(np.exp(t**3))

def F_t(t):
    return np.exp(t**3)

def trapezoidal(  fct_x, x0, xn, N):
    dx = float(xn-x0)/N
    f_Int  = 0.5*(fct_x( x0) + fct_x( xn)) + ( fct_x( x0+dx*np.arange(1, N, 1))).sum()
    return dx*f_Int

def midpoint( fct_x, x0, xn, N):
    dx     = float( xn-x0)/N
    a_xi   = x0 + 0.5*dx + np.arange( N)*dx
    f_int  = dx*( fct_x(a_xi)).sum()
    return f_int

#INTEGRAL COMPUTATIONS
trap = trapezoidal(f_t,0,1,100)
mid = midpoint(f_t,0,1,100)
exact_sol = F_t(1) - F_t(0)

print('Exact Solution: ' + str(exact_sol))
print('Trapezoidal Solution: ' + str(trap))
print('Midpoint Solution: ' + str(mid))